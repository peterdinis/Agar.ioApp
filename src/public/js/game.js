  ...
  return {
    ...
    setupSocketEvents() {
      ...
      this.canvas.addEventListener('mousemove', (e) => {
        ...
        if (now - lastMouseEmit < mouseThrottle || !this.socket || !this.currentPlayer) {
          return;
        }
    ...
    isObjectVisible(obj, viewport) {
      return obj.x + obj.radius >= viewport.left &&
    ...
    updateDebugInfo() {
      ...
      if (playerCounter) playerCounter.textContent = Object.keys(this.players).length;
      if (foodCounter) foodCounter.textContent = Object.keys(this.foods).length;
      if (visibleCounter) {
        const totalVisible = this.visibleObjects.players.size +
